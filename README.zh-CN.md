...
相对输入区域的归一化矩形、包含关系、置信度、耗时和标注预览图路径。本地布局图还会
生成 `right-of`、`below`、边缘对齐、投影重叠和自适应间距关系，并据此生成尺寸可变的
组合框。组合签名不包含元素宽度、高度和当前内容，因此名称变长、消息换行或内容增高后
仍属于相同结构；垂直重复列表项和相邻表格行不会被误合并。它不会遍历 UIA、运行 OCR、
理解控件含义、下载模型或调用远程 API。
